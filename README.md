...
| `TEAMCITY_CONSOLE_PROJECT_ID` | No | Console project whose build configurations are included in pending-change and active-build activity; defaults to `MainRepository_Venio` |
...
The **Delivery systems** page combines GitLab pipeline activity, TeamCity component activity, GitLab Runner availability, and TeamCity Agent status. It defaults to the version in `CURRENT_RELEASE`. Operators can enter another version such as `11.8.6.0` or `v11.8.6.0` to check GitLab and TeamCity without changing `.env`.

For TeamCity, the page checks Console, API, and Web for pending changes on the selected release branch. API and Web are checked directly. For Console, Pulseboard checks every build configuration in `TEAMCITY_CONSOLE_PROJECT_ID` together with the setup configuration's snapshot-dependency graph, so sibling project changes and changes hidden from the parent configuration are shown. Duplicate commits present in more than one configuration are counted once. Active Console builds are collected from the same configuration set. Console trigger requests also pass the numeric release version as both `reverse.dep.*.system.Version` and `system.Version` TeamCity properties.

**Trigger build** appears when that component has pending changes and no queued or running build on the selected release branch. For a custom version, the confirmation identifies both the exact custom branch and the configured release. The API independently validates custom versions, rechecks pending activity on that branch, and supplies the validated branch to TeamCity. Console triggers the setup build, allowing TeamCity to run its required dependency chain. Duplicate submissions are rejected and the dashboard polls the queued build through completion.
