...
For TeamCity, the page checks Console, API, and Web for pending changes on the selected release branch. API and Web are checked directly. For Console, Pulseboard reads the setup configuration's snapshot-dependency graph and checks the setup plus every dependency configuration that the Console trigger actually builds. Unrelated sibling configurations are excluded because triggering the setup cannot consume their pending changes. Duplicate commits present in more than one dependency are counted once, and active Console builds are collected across the same graph. Console trigger requests also pass the numeric release version as both `reverse.dep.*.system.Version` and `system.Version` TeamCity properties.
...
Run it after the Docker workflow finishes whenever `main` changes. With no argument it pulls and deploys `latest`; pass the commit SHA shown in the Docker/Discord notification to pin a specific published image:
